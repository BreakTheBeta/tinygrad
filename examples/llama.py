...
def concat_weights(models):
  def convert(name) -> Tensor:
    ...
    if len(disk_tensors) == 1 or len(disk_tensors[0].shape) == 1:
      print(disk_tensors[0].dtype, name)
      return disk_tensors[0].bitcast(dtypes.uint16).to(device=Device.DEFAULT) if disk_tensors[0].dtype == dtypes.bfloat16 else disk_tensors[0].to(device=Device.DEFAULT)
    print('--- big boy ---')
    print(disk_tensors[0].dtype, name)
...
class LLaMa:
  ...
  def build(model_path, tokenizer_path, model_gen="1", model_size="7B", quantize=False):
    ...
    weights = {k:v.cast(dtypes.uint32).mul(1<<16).bitcast(dtypes.float32).cast(dtypes.float16) if k != "rope.freqs" else v for k,v in weights.items()}
